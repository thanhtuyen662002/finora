...
export async function revalidateCategoryToken(
  ...
  try {
    ...
    if (error) {
      // Query error maps to UNRESOLVED (not STALE)
      return { status: 'UNRESOLVED' };
    }

    if (!data) {
      // Successful query returning no active row for a previously mapped candidate -> STALE
      return { status: 'STALE' };
    }

    return { status: 'RESOLVED', categoryId: data.id };
  } catch {
    // Thrown exception maps to UNRESOLVED (not STALE)
    return { status: 'UNRESOLVED' };
  }
}
