...
const oldHeader = `<PageHeader
          ...
          description={
            selectedCurrency === 'BASE' && data.autoFxEnabled
              ? \`Tổng hợp tự động quy đổi về \${data.baseCurrency} (\${data.dateRangeLabel})\`
              : \`Báo cáo dòng tiền theo \${selectedCurrency} (\${data.dateRangeLabel})\`
          }`;
...
const newHeader = `<PageHeader
        ...
        {selectedCurrency === 'BASE' && (data.baseValuation?.status === 'UNAVAILABLE' || data.baseHistorical?.status === 'UNAVAILABLE') && (
          <div className="bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 p-3 rounded-lg flex items-start gap-2 text-sm border border-amber-200 dark:border-amber-900/50">
            ...
            <Button variant="outline" size="sm" className="ml-auto shrink-0 bg-white dark:bg-transparent" onClick={() => loadData(activePeriod, selectedCurrency)}>
...
// add import for AlertCircle and RefreshCw if not there
if (!content.includes('AlertCircle')) {
  content = content.replace('import {', 'import { AlertCircle, RefreshCw,');
}
