...
check('LOCKFILE_COEXISTENCE', hasPackageLock && hasBunLock && !hasYarnLock && !hasPnpmLock, 'package-lock.json and bun.lock must coexist without yarn/pnpm');
...
const hasPrivacyDisclosure = pickerContent.includes('Finora không lưu ảnh hóa đơn');
check(
  'PRIVACY_DISCLOSURE_PRESENT',
  hasPrivacyDisclosure,
