...
- Authenticated server-only AI parsing
...
- **Phase 12A Test Suite (`tests/phase12a-transaction-draft.test.ts`)**: 20/20 PASS
  ...
  3. Zero coercion strictly enforced (numeric amount rejected, string amount accepted)
  4. Supported currency gate strictly enforced (VND, USD, EUR, JPY, CNY, KRW)
  5. Strict calendar date validator (Gregorian leap years, month boundaries)
  6. Candidate tokens validated against regex (UUIDs rejected at model boundary)
  7. Income stream database contract (`income_source_id`) and parent matching verified
  8. Currency precedence strictly enforced (Explicit > Account > Base fallback)
  9. Candidate overflow failsafe (bounded query CAP + 1, fail closed, clear array)
  10. Candidate label sanitization (control characters, length bound)
  11. Prompt construction (temporal context, zero UUIDs, adversarial defenses)
