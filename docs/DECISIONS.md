...
## ADR-016 — Application-Level AES-256-GCM Credential Encryption and Private Schema Architecture

**Status:** Accepted

**Decision:**
1. **Application-Level Envelope Encryption:** AI API credentials (e.g. Gemini) are never stored in plaintext or database-level reversible column encryption without application authentication. Credentials are encrypted at the application tier via standard Node.js `crypto.createCipheriv('aes-256-gcm')`.
2. **Cryptographic Standards:**
   - Algorithm: AES-256-GCM (Authenticated Encryption with Associated Data).
   - Key: 256-bit (32-byte) master key resolved from a server-side JSON key ring (`FINORA_AI_CREDENTIAL_KEY_RING_JSON`).
   - Nonce: 96-bit (12-byte) cryptographically secure random bytes generated per encryption via `crypto.randomBytes(12)`. Nonce reuse is strictly prohibited.
   - Authentication Tag: 128-bit (16-byte) authentication tag generated by GCM mode.
   - Canonical AAD Binding: Associated data binds `v${envelopeVersion}|${credentialId}|${ownerUserId}|${provider}|${source}`. Any cross-slot, cross-row, or version transplant renders decryption impossible and fails closed with `AI_CREDENTIAL_CORRUPTED`.
3. **Database Isolation (`private` Schema):**
   - Credentials table `private.ai_credentials` resides exclusively in the unexposed `private` schema.
   - PostgREST Data API exposure is blocked: schema `private` is completely revoked from `PUBLIC`, `anon`, and `authenticated`.
   - Table privileges to `service_role` are strictly least-privilege: `SELECT, INSERT, UPDATE`. `DELETE` and `TRUNCATE` are withheld.
4. **Service-Role RPC Facade:**
   - Three dedicated RPCs in the `public` schema (`ai_credentials_read_for_service`, `ai_credentials_write_for_service`, `ai_credentials_revoke_for_service`) provide the only programmatic gateway.
   - Functions are `SECURITY INVOKER`, with empty `SET search_path = ''`.
   - Browser execution (`PUBLIC`, `anon`, `authenticated`) is revoked. Only `service_role` is granted `EXECUTE`.
5. **Fail-Closed Source Resolution Priority:**
   - Priority sequence: `PERSONAL` > `ADMIN_ASSIGNED` > `SYSTEM`.
   - If a configured source fails integrity verification or is corrupted, resolution fails closed with `AI_CREDENTIAL_CORRUPTED` or `AI_CREDENTIAL_KEY_UNAVAILABLE` without falling through to a lower source.
6. **Strict Admin Authorization by UUID:**
   - Admin status is strictly determined by membership in `FINORA_ADMIN_USER_IDS` (comma-separated UUID allowlist).
   - Email addresses, JWT metadata, and client-supplied parameters are completely ignored for authorization decisions.

**Reason:**
- Completely eliminates plaintext API key exposure in databases, backups, or browser clients.
- Prevents cryptographic transplant attacks across users, providers, or sources.
- Protects operational and user-provided API keys in compliance with Master Project Instruction Section 25.

**Consequences:**
- Zero plaintext credential columns exist in PostgreSQL.
