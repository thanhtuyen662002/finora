...
User Selects Image (Camera / File)
       ↓
[Client Preflight Check] (Reject if file.size > 4,194,304 bytes with RECEIPT_FILE_TOO_LARGE)
       ↓
Browser Local Preview (Object URL) & Privacy Disclosure Display
       ↓
User Explicit Click: "Phân tích hóa đơn"
       ↓
[Authenticated Server Action Gate] (auth.getUser() session check; MUST succeed before any byte buffering or decoding)
       ↓
...
- **Supported User Flow:**
  ...
  9. The existing Phase 10 `AiRouter` executes the `receipt_vision` logical operation with provider-neutral media (exactly 1 HTTP attempt, zero auto-retry).
  10. The existing Phase 11 `AiCredentialProvider` resolves the appropriate active credential.
  11. The structured output is strictly validated by `AiOutputValidator<ReceiptVisionParseOutput>` against the exact 11-key schema and state consistency rules.
  12. Server exact-money canonicalizer converts lexical amount to canonical `numeric(20,4)` string.
  13. The server performs domain semantic validation, derives warnings deterministically from provider state fields, and computes `can_apply`.
...
2. **Next.js Framework Config (Numeric Integer):** Pass 12B-1 will configure `next.config.ts`:
   ```typescript
   experimental: {
     serverActions: {
...
Pass 12B-1 is authorized to add exactly one new production dependency:
- **Dependency:** `sharp`
...
- **Version & Runtime Policy:** At implementation time, `sharp` must be pinned to an exact reviewed version compatible with Node >= 22.
...
- AI credential repository / factory instantiation where avoidable;
...
Current Phase 10 `AiBaseRequest` is text-oriented (`contents: request.prompt`). Phase 12B defines an **additive, provider-neutral multimodal extension**:
...
- Exactly **1 HTTP generation attempt** at the provider adapter level.
...
### 15.1 State Machine & Truthful Cancellation Semantics
```text
[NO_IMAGE]
...
- Local `Object URL` references are revoked.
...
### 15.2 Truthful Vietnamese UI Copy & Privacy Disclosure
...
### 16.2 Media-Safe Error Boundary Policy
For `receipt_vision`:
...
### 16.3 Privacy-Safe Telemetry Specification
...
10. **Apply Safety & Default Leakage:**
...
11. **UI Truthfulness & Cancellation:**
...
2. **Pass 12B-2 — Candidate Integration, Domain Cross-Validation & Form Application:**
   ...
   - Warning generator with deterministic provenance mapping, strict `can_apply` gating logic, modal form application without default leakage.
3. **Pass 12B-3 — Corrective, Security & Verifier Hardening:**
   - Test suite, automated architectural source verifier, static gates, synthetic error media redaction test.
...
- **Phase 12B Status:** `PASS_12B_1_CORRECTIVE_3_COMPLETE`
- **Pass 12B-1 Implementation:** `COMPLETE / TESTED (169 checks PASS)`
